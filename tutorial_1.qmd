...
format:
  ...
  pdf:
    pdf-engine: xelatex
    documentclass: scrartcl     # KOMA-Script article (European standard)
    mainfont: "TeX Gyre Heros"  # A clean Sans-Serif font (Helvetica clone, standard in TeX)
    sansfont: "TeX Gyre Heros"
    monofont: "TeX Gyre Cursor" # A clean Monospace font (Courier clone)
    geometry:
      - top=25mm
      - bottom=25mm
      - left=25mm
...
macvm@Macs-Virtual-Machine sopra_server_tutorial % echo $JAVA_HOME
/Library/Java/JavaVirtualMachines/temurin-17.jdk/Contents/Home
macvm@Macs-Virtual-Machine sopra_server_tutorial % which java
/Library/Java/JavaVirtualMachines/temurin-17.jdk/Contents/Home/bin/java
```

:::{.callout-note}
Checking `java -version` is usually enough to verify the installation.
:::
...
Deno and npm (2 TS to JS runtimes) are available in the repository: use `deno task dev` to run the development server locally (at default port `localhost:3000`).
...
run `direnv allow` to enable the automatic environment variable loading.
:::
...
:::{.callout-tip appearance=minimal collapse=false title="Best practices"}
...
**Briefly explain what Github workflows are / where they live in the repo / their structure**
...
:::{.callout-note appearance=minimal collapse=true title="Set-up Sonar cloud and connect it to the sever repository"}
